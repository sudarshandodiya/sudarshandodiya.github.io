...
1. You have a set of functions that provide useful abstractions over the OkHttp library, and you use them in multiple projects.
2. To make it easier to manage these sets of functions, you decide to create a separate project that contains the functions that are common across all the projects, and you name the library `common`. This library uses OkHttp version 4.9.3 as its dependency.
3. In your project, you import the `common` library, but instead of using the version 4.9.3 that you are using in the `common` library, you use the version 4.10.0.
